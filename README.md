...
[![Version](https://img.shields.io/badge/版本-v2.11.0-green)](https://github.com/haremank/gitee-ai-agents-guide/releases/latest)
...
本地库默认使用**指定的本地目录**：在面板中设置一次保存目录后，媒体内容直接写为该文件夹里的真实文件，预览时默认从这个文件夹读取（读取顺序：目录文件 → 脚本存储 → 远端直链）；目录里还有一份 `library.json` 保存索引，与文件放在一起。未设置目录时，内容暂存在油猴脚本自身的存储（全站点共享同一份）。库标题栏显示当前存储位置，每条内容的卡片显示其来源站点。目录授权由浏览器按站点管理，使用本地库的网站需各自授权一次。图片、视频、音频、文本和 3D 文件可在面板内预览、搜索、按类型筛选、单独删除或一键清空。已取回本地的副本不依赖 Gitee/BOS 签名 URL，远程链接过期后仍能打开。旧版按站点隔离的 IndexedDB 数据会在对应站点首次打开面板时自动合并（每站点仅一次，原数据保留不删）。

在 Chrome / Edge 等支持 File System Access API 的浏览器中，可以设置一个固定的保存目录。新生成或导入成功的内容会优先写入该目录（写入失败时回退到脚本存储）；Firefox / Safari 等不支持的浏览器仍保存到脚本存储，并在面板中提示降级。单独删除或清空本地库时，脚本会同步尝试删除已写入目录的文件；如果目录授权已失效，会保留本地记录并提示重新授权，避免误以为文件已删除。
...
本地库的索引与文件随指定目录存续：目录里的内容在浏览器之外可见、可备份。脚本共享存储随 Tampermonkey 数据存续，同一浏览器全站点共用，但不跨浏览器、不跨设备同步。清空 / 重装 Tampermonkey 数据会删除脚本存储部分（指定目录中的文件不受影响），重要产物建议设置固定目录。
